...
# Ensure user is logged in and has a subscription context
function Ensure-AzContext {
    param(
        [string]$DesiredSubscriptionId
    )

    # 1. Make sure we have *some* context
    $ctx = Get-AzContext -ErrorAction SilentlyContinue

    if (-not $ctx -or -not $ctx.Account) {
        Write-Host "You are not logged in to Azure. Please sign in..." -ForegroundColor Yellow
    ...
    # 2. If caller specified a subscription, try to switch to it
    if ($DesiredSubscriptionId) {
        if ($ctx.Subscription.Id -ne $DesiredSubscriptionId) {
            Write-Host "Switching to subscription $DesiredSubscriptionId..." -ForegroundColor Yellow
            try {
                Select-AzSubscription -SubscriptionId $DesiredSubscriptionId -ErrorAction Stop
                $ctx = Get-AzContext -ErrorAction Stop
            }
            catch {
                Write-Error "Failed to switch to subscription $DesiredSubscriptionId. Error: $($_.Exception.Message)"
                throw
            }
        }
    }
    else {
        # 3. Ask the user to confirm the current subscription
        $answer = Read-Host "Use this subscription for World of Workflows deployment? (Y/N)"

        if ($answer -notin @("Y","y","Yes","yes")) {
            Write-Host "Listing available subscriptions..." -ForegroundColor Yellow
            ...
            $choice = Read-Host "Enter the number of the subscription to use (or press Enter to cancel)"
            if ([string]::IsNullOrWhiteSpace($choice)) {
                throw "User cancelled subscription selection."
            }

            $index = [int]$choice - 1
            if ($index -lt 0 -or $index -ge $subs.Count) {
                throw "Invalid subscription selection."
            }

            $selected = $subs[$index]
            Write-Host "Switching to subscription $($selected.Name) ($($selected.Id))..." -ForegroundColor Yellow
            Select-AzSubscription -SubscriptionId $selected.Id -ErrorAction Stop
            $ctx = Get-AzContext -ErrorAction Stop
        }
    }
    ...
    if (-not $ctx.Subscription -or -not $ctx.Subscription.Id) {
        throw "Az context has no subscription. Please ensure your user has access to at least one subscription."
    }
...
# Ensure context is correct (this will prompt the user if needed)
$ctx = Ensure-AzContext -DesiredSubscriptionId $SubscriptionId
$SubscriptionId = $ctx.Subscription.Id

Write-Host "Using subscription: $($ctx.Subscription.Name) ($SubscriptionId)" -ForegroundColor Green

# Now safely resolve the Web App -> Resource Group
Write-Host "Looking up Web App '$SiteName'..." -ForegroundColor Cyan
try {
    $webApp = Get-AzWebApp -Name $SiteName -ErrorAction Stop
}
catch {
    Write-Error "Failed to find Web App '$SiteName' in subscription $SubscriptionId. Error: $($_.Exception.Message)"
    throw
}

$ResourceGroupName = $webApp.ResourceGroup
$WebAppName        = $webApp.Name   # same as $SiteName, but clearer

Write-Host "Resolved Web App:"
...
# For now, just write it to a file next to the script
$outputPath = Join-Path -Path (Get-Location) -ChildPath "WoWDeploymentContext.json"

Write-Host "Writing deployment context to $outputPath ..."
$deploymentContextJson | Out-File -FilePath $outputPath -Encoding utf8

Write-Host "CustomerSetup.ps1 completed successfully."